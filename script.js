...
filterClearBtn.addEventListener('click', getAllMovies);
applyFilterBtn.addEventListener('click', filterAll);
...
function starringFilter() {
    ...
    for (var i=0; i<movies.length; i++){
        for (var j=0; j<movies[i].starring.length; j++){
            // console.log(movies[i].starring[j]);
            var movie = movies[i];
...
function searchMovies() {
    ...
    for (var i=0; i<movies.length; i++){
        if (searchValue.toLowerCase() == movies[i].title.toLowerCase()){
            var movie = movies[i];
            moviesOutput.innerHTML += `
            <div class="col-3 p-5">
                <div class="card">
...
function filterAll() {
    ...
    for (var i=0; i < movies.length; i++) {
        var movie = movies[i];  
        if (genre.value === movie.genre && rating.value === movie.rating) {
            moviesOutput.innerHTML += `
        <div class="col-3 p-5">
            <div class="card">
                <img src="${movie.image}" class="card-img-top" alt="Movie Poster">
